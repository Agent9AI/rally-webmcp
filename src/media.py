...
The lyric must teach these ideas in musical language: WebMCP lets a website expose named, structured browser tools so an agent does not have to guess at pixels; Rally uses those tools to stage a song, prepare an Insights article, propose safe connector onboarding, and read back the human's edits; the agent may inspect, prepare, and review, but the human confirms. Tell the publishing story in plain language: after explicit approval, one allowlisted workflow creates a draft rather than silently publishing. Keep adjacent protocol roles accurate: Rally's governed server-side MCP gateway connects approved business systems, A2A v1.0 handles outside-agent handoffs, and Rally keeps authority, chain of custody, and independent verification across them. Do not imply certification or endorsement.

Technical receipt context, not a required brand list for the lyric: the approved publishing route is one n8n MCP workflow creating an EmDash journal draft on agent9.dev's Cloudflare Workers and D1 site; connector profiles include n8n, Google Workspace, Slack, GitHub, Cloudflare, BigQuery, and other approved systems; Rally advertises A2A through its public Agent Card and JSON-RPC and HTTP+JSON interfaces. Preserve those facts in the receipt and review, but do not turn the song into a sequence of provider names.
